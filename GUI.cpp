...
void UpdateGUI(GameState& game, UIState& ui) {
    ...
    if (ui.currentScreen == 0) {
        ...
        if (game.matchStatus == 0) { //Isplaying
            if (game.inputType == 0) {
            ...
            if (confirm) {
                if (ui.endGameSelection == 0) {
                    ...
                    ui.currentScreen = 0; // back to menu
                }
            }
        }
...
void DrawGUI(const GameState& game, const UIState& ui) {
    ...
    // ----------------------------------------------------------
        ...
        DrawText("THONG TIN VAN DAU", 820, 30, 30, BLACK);
        DrawText(TextFormat("--- VONG CHOI %d ---", game.roundCount), 840, 70, 25, DARKBLUE); 
        
        if (game.matchStatus == 0) {
            if (game.isPlayer1Turn) DrawText(TextFormat(">> LUOT CUA: %s (X) <<", game.player1.name), 790, 110, 30, RED);
            else DrawText(TextFormat(">> LUOT CUA: %s (O) <<", game.player2.name), 790, 110, 30, BLUE);
        }

        if (game.inputType == 0) DrawText("Dieu khien: Chuot", 880, 150, 20, GRAY);
        else DrawText("Dieu khien: WASD + Enter", 850, 150, 20, GRAY);
       ...
       if (game.matchStatus != 0) {
            ...
            if (game.matchStatus == 1 || game.matchStatus == 2) {
                // caculate the point of start and end
                float startX = ui.cellStartX + game.winStartCol * ui.cellSize + ui.cellSize / 2.0f;
                float startY = ui.cellStartY + game.winStartRow * ui.cellSize + ui.cellSize / 2.0f;
                float endX = ui.cellStartX + game.winEndCol * ui.cellSize + ui.cellSize / 2.0f;
                float endY = ui.cellStartY + game.winEndRow * ui.cellSize + ui.cellSize / 2.0f;

                
                Color lineColor = (game.matchStatus == 1) ? MAROON : DARKBLUE;
                
                // draw the line
                DrawLineEx({startX, startY}, {endX, endY}, 12.0f, lineColor);
                
                // bo goc
                DrawCircle(startX, startY, 6.0f, lineColor);
                DrawCircle(endX, endY, 6.0f, lineColor);
            }
...
>>>>>>> 52f1be8450f0aa488f74e9dc9d4bafc73234087b
        }
    }
    // ----------------------------------------------------------
    // Màn hình 2: Settings
